...
unsigned int flash_size = 0, user_size = 0;
...
unsigned int find_user_end(void)
{
	...
	user = (const unsigned int *)(FLASH_START + USER_START);
	end = (const unsigned int *)(FLASH_START + flash_size);

	/* Last non-FF value is an user's firmware end */
	for (ptr = user, pos = 0; ptr < end; ptr++, pos += 4)
	{
		if (*ptr != 0xFFFFFFFFu)
		{
			res = pos + 4;
...
int boot_command(int cmd, unsigned char *buffer, int in_bytes, int buffer_size)
{
	...
	switch (cmd)
	{
		case 'i':
			...
			app_size = find_user_end();
			ver = find_user_ver();
			tlv_add_int(&p, e, 's', app_size);
			tlv_add_str(&p, e, 'v', ver);
			tlv_add_str(&p, e, 'b', bootloader_ver);
			tlv_add_int(&p, e, 'f', flash_size);
			break;
		case 'r':
			/* Read user area */
		...
		case 'e':
			/* Erase user area */
			flash_erase_user(USER_START, user_size);
			tlv_add_int(&p, e, 'r', 1);
			break;
		case 'w':
			/* Write user area */
...
int main(void)
{
	...
	user_size = flash_size - USER_START;

	// rcc_high_performance(XTAL_8MHZ);

	uart_open(BOOT_UART, BOOT_SPEED, 8, BOOT_PARITY, BOOT_STOPBITS, BOOT_RXD, BOOT_TXD, BOOT_RS485PIN);
